        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class=" mb-12 fade-in">
                ...
                <p class="text-xl text-gray-600">
                    {{ $category->description ?? 'Explore fonts in this category' }}
                </p>
            </div>

            <!-- Search and Sort Form (same as All Fonts page) -->
            <form method="GET" class="flex flex-col md:flex-row justify-between mb-8 gap-4">
                ...
                <select name="sort" class="w-full md:w-auto p-3 rounded-lg border border-gray-300 focus:border-indigo-500 transition">
                    ...
                    <option value="date_asc" {{ request('sort') === 'date_asc' ? 'selected' : '' }}>Sort by Date (Oldest First)</option>
                    <option value="date_desc" {{ request('sort') === 'date_desc' ? 'selected' : '' }}>Sort by Date (Newest First)</option>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                @forelse ($fonts as $font)
                    <div class="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-xl transition-shadow duration-300 fade-in">
                        ...
                        <div class="p-6">
                            ...
                            <span class="text-gray-600 mb-4 block">
                                Category: {{ $font->category->category_name ?? 'Uncategorized' }}
                            </span>
                            <p class="text-gray-600 mb-4">{{ Str::limit($font->description, 100) }}</p>
                            <div class="flex justify-between text-sm text-gray-500">
                                <span>Designer: {{ $font->designer ?? 'Unknown' }}</span>
                                <span>Avg Rating: {{ number_format($font->feedbacks->avg('rating') ?? 0, 1) }}/5</span>
                            </div>
                            <a href="{{ route('fonts.show', $font) }}" 
                               class="mt-4 block text-indigo-600 font-medium hover:underline">
                                View Details →
                            </a>
                        </div>
                    </div>
                @empty
                    <p class="text-center text-gray-600 col-span-full py-12">
                        No fonts found in this category. Be the first to share one!
                    </p>
                @endforelse
            </div>
